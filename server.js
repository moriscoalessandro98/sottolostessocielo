...
app.post('/api/dream', async (req, res) => {
  try {
    const { connectionId, dream } = req.body;
    if (!dream || !connectionId) return res.status(400).send('Manca dream o connectionId');
    await Dream.create({ connectionId, dream });
    res.sendStatus(200);
  } catch (err) {
    console.error(err);
...
function drawNebula(x,y,color){
  ...
  g.addColorStop(0,color); g.addColorStop(0.6,'rgba(0,0,0,0.05)'); g.addColorStop(1,'rgba(0,0,0,0)');
...
function drawPeople(){
  ...
  const groundY = H/2 + 120; // più in alto, vicino al centro
  const pw = 100, ph = 240;
  ...
  ctx.save();
  ctx.shadowColor = 'rgba(255,255,255,0.8)'; // bianco luminoso
  ctx.shadowBlur = 20; 
  ctx.drawImage(leftPerson, 80, groundY-ph, pw, ph);
  ctx.restore();
